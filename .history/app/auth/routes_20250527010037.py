...
@auth.route('/upgrade_account', methods=['POST'])
@login_required
def upgrade_account():
    if current_user.role == 'professional':
        flash('Ya eres usuario profesional.', 'info')
        return redirect(url_for('profile.edit_profile'))
    # Aquí deberías validar el pago antes de seguir (stripe, paypal, etc.)
    current_user.role = 'professional'
    db.session.commit()
    flash('¡Felicidades! Tu cuenta ahora es profesional y puedes publicar tus servicios.', 'success')
    return redirect(url_for('profile.edit_profile'))
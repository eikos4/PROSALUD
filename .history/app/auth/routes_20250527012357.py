...
@auth.route('/upgrade-account', methods=['GET','POST'])
@login_required
def upgrade_account():
    form = UpgradeAccountForm()
    if form.validate_on_submit():
        # … lógica para actualizar rol …
        flash('Tu cuenta ahora es profesional', 'success')
        return redirect(url_for('auth.edit_profile'))
    # si es GET o hay errores, volvemos a mostrar el perfil
    return render_template('edit_profile.html',
                           form=EditProfileForm(),    # tu form de perfil
                           upgrade_form=form)
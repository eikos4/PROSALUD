...
def request_detail(request_id):
    ...
    # Ejemplo: obtén los mensajes asociados a la solicitud, si tienes ese modelo
    # Si no tienes mensajes, puedes dejar: messages = []
    messages = []
    form = MessageForm()
    if form.validate_on_submit():
        # Lógica para guardar mensaje, si tienes modelo Message
        msg = Message(
            sender_id=current_user.id,
            receiver_id=req.service.professional_id,
            body=form.body.data,
            # request_id = request_id, # solo si tu modelo Message lo tiene
        )
        db.session.add(msg)
        db.session.commit()

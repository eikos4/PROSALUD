...
def evaluate(request_id):
    req = Request.query.get_or_404(request_id)
    # Solo puedes evaluar si la solicitud es tuya y está completada
    if req.client_id != current_user.id or req.status != "completed":
        flash("No puedes evaluar este servicio.", "danger")
        return redirect(url_for('client.my_requests'))
    # Evita doble evaluación
    existing = Evaluation.query.filter_by(client_id=current_user.id, professional_id=req.service.professional_id).first()
    if existing:
    ...
    if form.validate_on_submit():
        evaluation = Evaluation(
            professional_id=req.service.professional_id,
            client_id=current_user.id,
        ...
        db.session.commit()
        flash("¡Reseña enviada con éxito!", "success")
        return redirect(url_for('client.my_requests'))

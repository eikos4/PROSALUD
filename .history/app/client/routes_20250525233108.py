...
def request_detail(request_id):
    ...
    if current_user.id == req.client_id:
        other_user = req.service.professional  # asumiendo que tienes este campo
    else:
        other_user = req.client

    # Traer la conversación entre current_user y other_user
    ...
    # Pasa `other_user`, `convo` y `form` al template
    return render_template(
        'client/request_detail.html',
            req=req,
            other_user=other_user,  # Usuario con quien conversas
            convo=convo,            # Lista de mensajes
            form=form,              # Formulario para enviar mensajes
            evaluation_exists=evaluation_exists,  # Si ya se evaluó
)

...
@login_required
def my_requests():
    # Todas las solicitudes del cliente actual
    requests = Request.query.filter_by(client_id=current_user.id).order_by(Request.date_requested.desc()).all()
...
def request_detail(request_id):
    ...
    # Ejemplo: obtén mensajes asociados si tienes un modelo Message asociado a la solicitud
    messages = []  # Reemplaza esto por tu lógica real si usas mensajes por solicitud
    form = MessageForm()
    if form.validate_on_submit():
        # Lógica para guardar un mensaje nuevo, etc.
        pass
    return render_template(
